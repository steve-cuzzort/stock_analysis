...
import hibernate.HibernateUtilSQLite;
...
public class LoadFromYahoo 
{
	...
	{		
		loadStockFromYahoo(stock, from, LocalDate.now().toDateTimeAtStartOfDay());
	}

	public static void loadStockFromYahoo(Stock stock, org.joda.time.DateTime from, org.joda.time.DateTime to)
	{
		org.joda.time.DateTime today = LocalDate.now().toDateTimeAtStartOfDay();
	...
	{
		return LocalDate.parse(sdate).toDateTimeAtStartOfDay();
	}
